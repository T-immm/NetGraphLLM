...
def expand_mask(mask: torch.Tensor, dtype: torch.dtype, tgt_len: Optional[int] = None):
    ...
    # print('mask_in_B---------{}'.format(mask.shape))
    
    tgt_len = tgt_len if tgt_len is not None else src_len

    # print('tgt---------{}'.format(tgt_len))
    
    expanded_mask = mask[:, None, None, :].expand(bsz, 1, tgt_len, src_len).to(dtype)

    # print('expanded_mask---------{}'.format(expanded_mask.shape))
    
    inverted_mask = 1.0 - expanded_mask
    
    # print('inverted_mask---------{}'.format(inverted_mask.shape))
...
def prepare_decoder_attention_mask(attention_mask, input_shape, inputs_embeds):
    ...
    if attention_mask is not None:
        ...
        # print('expanded_attn_mask---------{}'.format(expanded_attn_mask.shape))
...
def batch_subgraph_pair_level(edge_index,
    ...
    for node_idx in node_ids:
        ...
        # print('AAA edge_index-----------{}'.format(edge_index_sub))
        
        edge_attr_temp = edge_attr[node_idx: node_idx + len(edge_index_sub[0])]
        # print('Temp-----{}'.format(edge_attr_temp))
        edge_attr_sub.append(edge_attr_temp)
        ...
        # print('BBB edge_index-----------{}'.format(edge_index_sub))
        
        # Batching Graph
        edge_index_sub += inc_num

        # print('CCC edge_index-----------{}'.format(edge_index_sub))
    ...
    subset, mapping, batch = torch.cat(subset_list), torch.as_tensor(mapping_list), torch.as_tensor(batch_list)
    edge_index_sub = torch.cat(edge_index_sub_list, dim=1)
    edge_attr_sub = torch.cat(edge_attr_sub)
    # print('SUB-----{}'.format(edge_attr_sub))

    # print(len(edge_index_sub[0]))
    # print(len(edge_attr_sub))
    
    return subset, edge_index_sub, edge_attr_sub, mapping, batch
