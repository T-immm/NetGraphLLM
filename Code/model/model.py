...
class GriTGraphAdapter(nn.Module):
    ...
    def forward(self, node_configurations, input_attn, edge_index, mapping, input_node_pair_embed, batch):
        ...
        # print('config----{}'.format(node_configurations.shape))
...
class GritAdapterLayer(nn.Module):
    ...
    def forward(self, query, edge_index, mapping, input_node_pair_embed, batch):
        ...
        # changed
        # input_node_pair_embed = input_node_pair_embed.bfloat16()
        # node_pos = node_pos.bfloat16()
        ...
        if self.task_level == 'graph':
            ...
            for query_idx in range(query.shape[1]):
                ...
                # changed
                # x_out = x_out.bfloat16()
                
                for layer in self.mp_layer:
                    x_out, e_out = layer(x=x_out, edge_index=edge_index, input_node_pair_embed=e_out)
                out_list.append(x_out)
...
class Transformer(nn.Module):
    ...
    def forward(self, input_ids, labels, node_ids, attention_mask=None, start_pos=0):
        ...
        eval_pred = tokenizer.batch_decode(predicted_tokens)
        eval_label = tokenizer.batch_decode(filtered_labels)
        
        eval_pred = [item.split('</s>')[0] for item in eval_pred]
        eval_pred = [item.split('\n\n###\n\n ')[-1] for item in eval_pred]
        
        eval_label = [item.split('</s>')[0] for item in eval_label]
        eval_label = [item.split('\n\n###\n\n ')[-1] for item in eval_label]
        
        eval_pred = self.convert_data(eval_pred)
        eval_label = self.convert_data(eval_label)
        
        print('Pre---{}'.format(eval_pred))
        print('label---{}'.format(eval_label))
        
        
        for i in range(0, len(eval_pred)):
            if eval_pred[i] == eval_label[i]:
                val_acc = val_acc + 1
        
        
        val_acc = val_acc / _bsz
        print('Acc---{}'.format(val_acc))"""
    ...
    def convert_data(self, data_list):
        ...
        for item in data_list:
            if item.isdigit():
                ...
                temp_list.append(int(current_number))  
                current_number = ""  
            elif item == ']' and current_number:
                temp_list.append(int(current_number))  
    ...
    def graphForward(self, node_ids):
        ...
        if self.params.task_level == 'graph':
            ...
            subset, edge_index_sub, edge_attr_sub, mapping, batch = batch_subgraph_pair_level(self.edge_index, self.edge_attr,
                                                                        node_ids, num_nodes=self.node_configuration_ids.shape[0],)
        ...
        print("ttttest111--{}".format(node_configurations_embeds.shape))
        node_configurations_embeds = self.attention_encoder(node_configurations_embeds, adapter_input_attn)
        print("ttttest222--{}".format(node_configurations_embeds.shape))
    ...
    def set_trainable_params_new(self):
        ...
        # adapter = ["up_projection", "down_projection", "attention_encoder"]
...
class CrossAttentionLayer(nn.Module):
    ...
    def forward(self, query, input_embeds, input_attn):
        ...
        # changed
        # input_attn = input_attn.bfloat16()
